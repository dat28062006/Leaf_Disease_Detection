"""
new_model/train.py - Huấn luyện mô hình nâng cấp (EfficientNet + DAAM + Swin Transformer)
...
train_tf = A.Compose([
    ...
    A.RandomRain(p=0.2, drop_length=20, drop_width=1, blur_value=3),  # giọt mưa
    A.Spatter(p=0.2),                                                  # bùn đất
    A.RandomShadow(p=0.2),                                             # bóng râm
    A.CoarseDropout(max_holes=8, max_height=32, max_width=32,
                    min_holes=1, min_height=8, min_width=8,
                    fill_value=0, p=0.2),                              # côn trùng/vết cắn
...
# ─────────────────────────────────────────────
# Model, Loss, Optimizer, Scheduler, Mixup
# ─────────────────────────────────────────────
...
for epoch in range(start_epoch, EPOCHS):
    ...
    for imgs, y in pbar:
        ...
        imgs, y = mixup_fn(imgs, y)                    # Mixup / CutMix
        optimizer.zero_grad()
        with torch.amp.autocast("cuda"):
            out  = model(imgs)
